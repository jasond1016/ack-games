...
export function updatePhysicalVehicle({
  ...
  if (planarSpeed > 0.001) {
    const rollingScale = Math.max(0.5, Number.isFinite(rollingDragScale) ? rollingDragScale : 1);
    const dragMagnitude = config.rollingDrag * rollingScale
      + config.aerodynamicDrag * planarSpeed * planarSpeed;
    chassis.addForce({
