...
function startVacuumGame(updateHistory = true) {
  homeView.hidden = true;
  gameView.hidden = false;
  document.title = "吸尘器接管道 - ACK Games";
  ...
  if (updateHistory) {
    history.pushState({ view: "vacuum" }, "", "#vacuum");
  }
}
...
window.addEventListener("popstate", () => {
  if (location.hash === "#vacuum") {
    startVacuumGame(false);
  } else {
    showHome(false);
  }
});

vacuumGameCard.addEventListener("click", () => startVacuumGame());
