...
test("Cloudflare asset graph is fail-closed and excludes repository internals", () => {
  ...
  assert.ok(plan.pagesFiles.includes("racing-audio.mjs"));
  ...
  assert.ok(plan.pagesFiles.includes("racing-surface-contact.mjs"));
  assert.ok(plan.pagesFiles.includes("racing-tire-dynamics.mjs"));
  assert.ok(plan.pagesFiles.includes("racing-wheel-animation.mjs"));
  assert.ok(plan.pagesDirectories.includes("assets/freedrive"));
